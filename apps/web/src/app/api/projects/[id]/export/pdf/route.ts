...
import { buildCalculationPdf } from "@/lib/export/pdf";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** Exportera senaste (eller ?resultId=) kalkyl till PDF. */
export async function GET(
  ...
  const buffer = await buildCalculationPdf(data);
  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="kalkyl-${safeFilename(data.projektNamn)}.pdf"`,
    },
  });
}
